#!/bin/bash
NUM_GPUS_PER_NODE=1
NUM_WORKERS=6  # max is 6/7 for tile size 512 and Aperio, 4 for AP+P1000
# Mlflow configs
...
TARGET="outcome"
OUTER_SPLIT=4
EXPERIMENT_NAME="Folds_correct_outer_cross${OUTER_SPLIT}_val_int${DS_NAME}_${TILE_SIZE}_${BATCH_SIZE}"  # new, still running
...
CLINICAL_FEATURES="['er','her2','grade','age_diagnose','pr']"  #'cox2','p16','p53','pr'
...
MACHINE_PARAMETERS="trainer.devices=$NUM_GPUS_PER_NODE"
export RUN_CONFIG

python ../../drop/engine/cls/trainer.py \
$RUN_CONFIG $MACHINE_PARAMETERS $DATA_PARAMETERS $MLFLOW_PARAMETERS $HYPERPARAMETERS $INTRGRATIVE_CFG