...
TARGET="outcome"
OUTER_SPLIT=4
EXPERIMENT_NAME="Folds_correct_outer_cross${OUTER_SPLIT}_val_img${DS_NAME}_${TILE_SIZE}_${BATCH_SIZE}"  # change this every time you run the script
...
RUN_CONFIG="task=cls_mil/tile_dataset \
  ...
  experiment.use_specific_folds=[3,4] \
...
$RUN_CONFIG $MACHINE_PARAMETERS $DATA_PARAMETERS $MLFLOW_PARAMETERS $HYPERPARAMETERS