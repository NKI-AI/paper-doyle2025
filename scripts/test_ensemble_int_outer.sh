...
TARGET="outcome"
OUTER_SPLIT=3
EXPERIMENT_NAME="Folds_correct_outer_cross${OUTER_SPLIT}_val_int${DS_NAME}_${TILE_SIZE}_${BATCH_SIZE}"
echo $EXPERIMENT_NAME
MLFLOW_EXPERIMENT_NAME=$EXPERIMENT_NAME
MASTER_NODE=$(scontrol show hostnames "$SLURM_JOB_NODELIST")  # there is only one node at this point -- the master node
MLFLOW_PORT=43283
...
CLINICAL_FEATURES="['er','her2','grade','age_diagnose','pr']"
INTRGRATIVE_CFG="\
dataset.extra_features=$CLINICAL_FEATURES \
+model.extra_features=$CLINICAL_FEATURES \
...
python ../../drop/engine/cls/trainer.py $RUN_CONFIG $MACHINE_PARAMETERS $DATA_PARAMETERS $MLFLOW_PARAMETERS $HYPERPARAMETERS $INTRGRATIVE_CFG $ENSEMBLE_CONFIG
...
$RUN_CONFIG $MACHINE_PARAMETERS $DATA_PARAMETERS $MLFLOW_PARAMETERS $HYPERPARAMETERS $ENSEMBLE_CONFIG $INTRGRATIVE_CFG $SLOANE_PARAMS